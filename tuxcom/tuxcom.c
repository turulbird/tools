 ...
 ******************************************************************************/
 ...
 || defined HAVE_COOL_HARDWARE \
 || HAVE_TRIPLEDRAGON \
 || HAVE_SPARK_HARDWARE \
 || defined(HAVE_DUCKBOX_HARDWARE) \
 || HAVE_ARM_HARDWARE \
 || HAVE_MIPS_HARDWARE
int GetRCCode()
{
...
int main()
{
 ...
 || HAVE_TRIPLEDRAGON \
 || HAVE_SPARK_HARDWARE \
 || HAVE_DUCKBOX_HARDWARE \
 || HAVE_ARM_HARDWARE \
 || HAVE_MIPS_HARDWARE
	rc = open("/dev/input/nevis_ir", O_RDONLY);
#if defined(HAVE_SPARK_HARDWARE) \
 || defined(HAVE_DUCKBOX_HARDWARE) \
 || defined(HAVE_ARM_HARDWARE)
	if (rc < 0)
	{
		...
		if (firstentry == 1)
		{
			...
			if (szPass[0] != 0x00)
			{
				...
				*szP = 0x00;
				...
				if (GetInputString(250, 19, szP, info[INFO_PASS1 * NUM_LANG + language], YES) != RC_OK)
				{
					break;
				}
				if (strcmp(szP,szPass) != 0)
				{
					break;
				}
				RenderFrame(LEFTFRAME);
					...
					else
					{
							...
							switch (MessageBox(szMessage,(finfo[curframe].zipfile[0] == 0x00 ? info[INFO_COPY * NUM_LANG + language] : ""), (finfo[curframe].zipfile[0] == 0x00 ? OKHIDDENCANCEL : OKCANCEL )))
							{
								...
								case HIDDEN:
								{
								    for (pos = 0; pos < finfo[curframe].count; pos++)
								    {
										...
										{
											pfe = getfileentry(curframe, pos);
											if (DoCopy(pfe, HIDDEN, OVERWRITESKIPCANCEL, szZipCommand) < 0)
											{
												break;
											}
										}
						...
						else
						{
							...
							switch (MessageBox(szMessage,(finfo[curframe].zipfile[0] == 0x00 ? info[INFO_COPY * NUM_LANG + language] : ""),(finfo[curframe].zipfile[0] == 0x00 ? OKHIDDENCANCEL : OKCANCEL )))
							{
								case YES:
								{
									if (DoCopy(pfe,YES, OVERWRITECANCEL,szZipCommand) < 0)
									{
										break;
									}
									DoZipCopyEnd(szZipCommand);
				...
				if (tool[ACTION_MOVE - 1] == ACTION_MOVE)
				{
						...
						switch (MessageBox(szMessage, info[INFO_MOVE * NUM_LANG + language], OKHIDDENCANCEL))
						{
							case YES:
							{
								for (pos = 0; pos < finfo[curframe].count; pos++)
								{
									...
									{
										pfe = getfileentry(curframe, pos);
										if (DoMove(pfe, YES, OVERWRITESKIPCANCEL) < 0)
										{
											break;
										}
									}
							...
							case HIDDEN:
							{
								for (pos = 0; pos < finfo[curframe].count; pos++)
								{
									...
									{
										pfe = getfileentry(curframe, pos);
										if (DoMove(pfe, HIDDEN, OVERWRITESKIPCANCEL) < 0)
										{
											break;
										}
									}
					...
					else
					{
						...
						switch (MessageBox(szMessage, info[INFO_MOVE * NUM_LANG + language], OKHIDDENCANCEL))
						{
							case YES:
							{
								if (DoMove(pfe,YES,OVERWRITECANCEL) < 0)
								{
									break;
								}
								FillDir(1 - curframe, SELECT_NOCHANGE);
...
int MessageBox(const char* msg1, const char* msg2, int mode)
{
	...
	do
	{
		if ((GetRCCode(RC_NORMAL) == 0) && mode == CANCELRUN)
		{
			return NO;
		}
		switch (rccode)
...
int ShowProperties()
{
	...
	if (le1 + 4 * BORDERSIZE > wi)
	{
		wi = le1 + 4 * BORDERSIZE;
	}
	if (wi > viewx - 4 * BORDERSIZE)
	{
		wi = viewx - 4 * BORDERSIZE;
	}
	/* box */
...
int GetInputString(int width, int maxchars, char *str, char *message, int pass)
{
	...
	wi = MINBOX;
	if (width > viewx - 8 * BORDERSIZE)
	{
		width = viewx - 8* BORDERSIZE;
	}
	if (le1 > wi)
	{
		wi = le1 + 6 * BORDERSIZE;
		...
		if ((rccode >=0x20) && (rccode < 0x0100))
		{
			...
			if ((kbcode>=RC_0) && (kbcode<=RC_9))
			{
				...
				if (markmode == 0)
				{
					...
					if (pch == NULL)
					{
						szdst[pos] = (textuppercase == 0 ? numberchars[rccode][0] : toupper(numberchars[rccode][0]));
					}
					else
					{
						if (pch == &(numberchars[rccode][strlen(numberchars[rccode]) - 1]))
						{
							szdst[pos] = (textuppercase == 0 ? numberchars[rccode][0] : toupper(numberchars[rccode][0]));
						}
						else
						{
							szdst[pos] = (textuppercase == 0 ? *((char*)pch + 1) : toupper(*((char*)pch + 1)));
						}
					}
				}	
			}
...
//#endif
		...
		switch(rccode)
		{
			...
			case RC_DOWN:
			{
				if (markmode == 0)
				{
					...
					else
					{
						if (pch == charset)
						{
							szdst[pos] = charset[strlen(charset) - 1];
						}
						else
						{
							szdst[pos] = *((char*)pch - 1);
						}
					}
					prev_key = -1;
				}
			...
			case RC_UP:
			{
				if (markmode == 0)
				{
					...
					{
						if (pch == &(charset[strlen(charset) - 1]))
						{
							szdst[pos] = charset[0];
						}
						else
		...
		if (pos <  0)
		{
			pos = 0; // cannot happen, pos is unsigned :)
		}
#endif
		if (pos >= strlen(szdst))
		{
...
int DoCopy(struct fileentry *pfe, int typ, int checktype, char *szZipCommand)
{
	...
	if (finfo[curframe].ftpconn != NULL)
	{
		...
		if (FTPcmd(curframe, "SIZE ", pfe->name, buf) == 213)
		{
			size = atol(buf + 4);
		}
		else
	...
	while(1)
	{
		...
		if (st.st_size >= FILEBUFFER_SIZE) // File is to big to fit in Buffer, we have to read next/previous part
		{
			...
			if ((sel < 0 ) && (curcount > 0)) // read previous part
			{
				...
				if (filepos > 0)
				{
					fgets(szFileBuffer, FILEBUFFER_SIZE, pFile); // ignore first (probably incomplete) line
				}
				count = 0;
				p = szFileBuffer;
				pcur = szFileBuffer;
...
int CheckZip(char* szName)
{
	...
	if (strcmp((char*)(szName+len-4),".tar") == 0)
	{
		return TAR;
	}
	if (strcmp((char*)(szName+len-4),".ftp") == 0)
	{
		return FTP;
	}
	if (len < 6)
	...
	if (strcmp((char*)(szName+len-6),".tar.Z") == 0)
	{
		return COMPRESS;
	}
	if (len < 7)
	...
	if (strcmp((char*)(szName+len-7),".tar.gz") == 0)
	{
		return GZIP;
	}
	if (len < 8)
	...
	if (strcmp((char*)(szName+len-8),".tar.bz2") == 0)
	{
		return BZIP2;
	}
	return -1;
...
void ReadZip(int typ)
{
	...
	while (fgets(szLine, 400, _pipe))
	{
		...
		if (name[0] != 0x00)
		{
			...
			if (d == 'd')
			{
				pzfe1->fentry.st_mode |= S_IFDIR;
				pzfe1->fentry.st_size = 0;
			}
			if (r == 'r')
			{
				pzfe1->fentry.st_mode |= S_IRUSR;
			}
			if (w == 'w')
			{
				pzfe1->fentry.st_mode |= S_IWUSR;
			}
			if (x == 'x')
			{
				pzfe1->fentry.st_mode |= S_IXUSR;
			}
			if (pzfe2 == NULL)
			{
	...
	else
	{
		while (fgets(line, 128, fp))
		{
			...
			*p = 0;
			...
			if (!strcmp(line, "host") )
			{
				strcpy(finfo[curframe].ftphost, p);
			}
			else if (!strcmp(line, "port"))
			{
				finfo[curframe].ftpport = atoi(p);
			}
			else if (!strcmp(line, "user"))
			{
				strcpy(finfo[curframe].ftpuser, p);
			}
			else if (!strcmp(line, "pass"))
			{
				strcpy(finfo[curframe].ftppass, p);
			}
			else if (!strcmp(line, "dir"))
			{
				strcpy(dir, p);
	...
	*s = 0;
	...
	do
	{
		...
		if (name[0] != 0x00)
		{
			...
			while (!isspace(pname[strlen(pname) - 1]))
			{
				pname[strlen(pname) - 1] = 0x00;
			}
			if (pname[0] == '/')
			{
				pname++;
...
void SetPassword()
{
	...
	*szP1 = 0x00;
	*szP2 = 0x00;
	if (GetInputString(150, 19, szP1, info[INFO_PASS2 * NUM_LANG + language], NO) != RC_OK)
	{
		return;
	}
	if (GetInputString(150, 19, szP2, info[INFO_PASS3 * NUM_LANG + language], NO) != RC_OK)
	{
		return;
	}
...
void ReadSettings()
{
	...
	FillDir(1 - curframe,SELECT_NOCHANGE);
	FillDir(    curframe,SELECT_NOCHANGE);
	if (lfirst < finfo[LEFTFRAME ].count)
	{
		finfo[LEFTFRAME ].first = lfirst;
	}
	if (rfirst < finfo[RIGHTFRAME].count)
	{
		finfo[RIGHTFRAME].first = rfirst;
	}
