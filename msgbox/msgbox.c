...
int main (int argc, char **argv)
{
	...
	for (tv = 1; !dloop && tv < argc; tv++)
	{
		...
		if ((rptr = strchr(aptr, '=')) != NULL)
		{
			...
			else if (strstr(aptr, "title=") != NULL)
			{
			...
			else if (strstr(aptr, "timeout=") != NULL)
			{
			...
			else if (strstr(aptr, "popup=") != NULL)
			{
				dloop = Transform_Msg(rptr);
				if (timeout == 0)
				{
			...
			else if (strstr(aptr, "refresh=") != NULL)
			{
			...
			else if (strstr(aptr, "select=") != NULL)
			{
				dloop = GetSelection(rptr);
			}
			else if (strstr(aptr, "default=") != NULL)
			{
			...
			else if (strstr(aptr, "order=") != NULL)
			{
			...
			else if (strstr(aptr, "absolute=") != NULL)
			{
			...
			else if (strstr(aptr, "cyclic=") != NULL)
			{
