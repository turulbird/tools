 ...
 *
 * Version for DGStation Cuberevo receivers.
 *
 ...
 * Description:
 *
 * Debugged and enhanced version of eeprom_ipbox intended to be used
 * with DGStation CubeRevo models CubeRevo, mini, mini II, 200HD, 250HD
 * and their Vizyon and ABcom counterparts.
 * Some aspects have not been tested:
 * - operation on models 2000HD and 3000HD
 * - writing new values and keys (not compiled, can be enabled)
 */
...
#define CONFIG_CUBEREVO_MINI_FTA
#define CONFIG_CUBEREVO_250HD
...
static struct
{
	...
	char   *name;
	int    isRO;
} db_name_tbl[] =  // data is only stored under a numbered key, the names here are of our own making
{
 ...
 || defined(CONFIG_CUBEREVO_MINI_FTA) \
 || defined(CONFIG_CUBEREVO_250HD)  // TODO: 2000HD & 3000HD models?
...
#define DB_HEADER_SIZE   sizeof(db_item)
...
//	printf("[eeprom] %s > key = %d\n", __func__, (int)key);
	for (a = 0; a < db_name_tbl_size; a++)
	{
//		printf("[eeprom] Checking for key %d (name: %s)\n", db_name_tbl[a].key, db_name_tbl[a].name);
...
//			printf("[eeprom] key name %s found\n", *name);
	...
	*name = "unknown";
//	printf("[eeprom] %s < (not found)\n", __func__);
	return 1;
}

int eeprom_write(int fd, unsigned int dev_addr, unsigned int offset, unsigned char *buffer, unsigned int cnt)
{
	unsigned int end = offset + cnt;
...
int eeprom_read(int fd, unsigned int dev_addr, unsigned int offset, unsigned char *buffer, unsigned int cnt)
{
	unsigned int end = offset + cnt;
	unsigned int blk_off;
	int rcode = 0;
...
//	printf("[eeprom] item.len = %d\n", item.len);
	...
	if (item.len > 0 && buf != NULL)
	{
		...
		rcode = eeprom_read(fd, CFG_EEPROM_ADD, offset + DB_HEADER_SIZE, buf, buflen);
		if (rcode)
		{
			return -1;
		}
...
#ifdef write_works
int search_item(int fd, int offset, unsigned short key, unsigned char *buf, unsigned char buflen, int offret, int lenret)
{
	int rcode;
	int len;
	db_key keytmp;
	...
	do
	{
		len = read_item(fd,  offset, &keytmp, NULL, 0);
		if (len < 0)
		{
			return 1;
		}
	...
	if (offret != 0)
	{
		offret = offset - (DB_HEADER_SIZE + len);
	}
	if (lenret != 0)
	{
		lenret = len;
...
static int save_item(int fd, int offset, db_key key, const char *buf, int len)
{
	...
	if (buf != NULL && len != 0)
	{
		...
		rcode = eeprom_write(fd, CFG_EEPROM_ADD, offset + DB_HEADER_SIZE, (unsigned char *)buf, len);
...
static int del_item(int fd, db_key key)
{
	...
	do
	{
		...
		if (keytmp == key)
		{
			...
			next_offset = offset + DB_HEADER_SIZE + len;
			next_len = read_item(fd, next_offset, &next_keytmp, NULL, 0);
			if (next_len >= 0)
			{
				if (next_keytmp == db_key_null)
				{
					len += DB_HEADER_SIZE + next_len;
				}
			}
...
int add_item(int fd, db_key key, const char *data)
{
	...
	do
	{
		rcode = search_item(fd, offset, db_key_null, NULL, 0, offset, len);
		...
		if (len == data_len || len >= data_len + DB_HEADER_SIZE)
		{
			...
			if (len > data_len)
			{
				rcode = save_item(fd, offset + DB_HEADER_SIZE + data_len, db_key_null, NULL, len - data_len - DB_HEADER_SIZE);
			}
			return rcode;
		}
		offset += DB_HEADER_SIZE + len;
	...
	{
		offset += DB_HEADER_SIZE + data_len;
...
int main(int argc, char *argv[])
{
	...
	if (boxtype == NULL)
	{
		printf("Cannot determine boxtype, assuming cuberevo.\n");
	}
	else
	{
		buffer = malloc(EEPROM_SIZE);
		fscanf(boxtype, "%s", buffer);
		fclose(boxtype);
	}
	if ((strncmp(buffer, "cuberevo_250hd", 14) == 0)
	||  (strncmp(buffer, "cuberevo_mini_fta", 17) == 0))
	{
	...
	else if (argc == 2) // read key and display name and value or dump
	{
		if (strcmp(argv[1], "-h") == 0)
		{
			...
			rcode |= eeprom_read(fd_i2c, CFG_EEPROM_ADD + 1, 0, buf, 0x100);
			if (rcode < 0)
			{
				rcode = 1;
				goto failed;
		...
		do
		{
			...
			if (found || dump)
			{
				...
				printf("0x%02x    %s\t %2d\t%s%s\n", offset, name, rcode, (char *)((key == db_key_null) ? "-" : (char *)buf), ((found && dump) ? " < found" : ""));
				found = 0;
			}
			offset += DB_HEADER_SIZE + rcode;
		}
		while (key != db_key_end);
	}
	else  // no arguments; show usage
	{
		...
		printf(" Usage:\n");
#ifdef write_works
		printf("%s -d | -h | [key name [value]]\n\n", argv[0]);
#else
		printf("%s -d | -h | [key name]\n\n", argv[0]);
#endif
		printf(" no args: show this usage\n");
		printf(" -d: display EEPROM contents\n");
		printf(" -h: hex dump EEPROM contents\n");
		printf(" only key name: if present, show key name and value\n");
#ifdef write_works
		printf(" key name and value: write new value for key name\n");
#endif
	}
	return (tvmode ? tvmode : 0);

failed:
	printf("[eeprom] Error occurred\n");
	return -1;
}
// vim:ts=4
