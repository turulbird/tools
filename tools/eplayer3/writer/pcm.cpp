...
bool WriterPCM::prepareClipPlay()
{
	...
	switch (uBitsPerSample)
	{
		...
		default:
		{
			printf("[eplayer3] inappropriate bits per sample (%d) - must be 16 or 24\n", uBitsPerSample);
			return false;
		}
	}
	return true;
...
bool WriterPCM::Write(AVPacket *packet, int64_t pts)
{
	...
	if (restart_audio_resampling)
	{
		...
		if (!codec)
		{
			fprintf(stderr, "[eplayer3] %s %d: avcodec_find_decoder(%llx)\n", __func__, __LINE__, (unsigned long long) c->codec_id);
			return false;
		}
		avcodec_close(c);
		if (avcodec_open2(c, codec, NULL))
		{
			fprintf(stderr, "[eplayer3] %s %d: avcodec_open2 failed\n", __func__, __LINE__);
			return false;
		}
	}
	if (!swr)
	{
		...
		if (!swr)
		{
			fprintf(stderr, "[eplayer3] %s %d: swr_alloc failed\n", __func__, __LINE__);
			return false;
		}
		...
		if (e < 0)
		{
			fprintf(stderr, "[eplayer3] swr_init: %d (icl=%d ocl=%d isr=%d osr=%d isf=%d osf=%d)\n",
	...
	while (packet_size > 0 || (!packet_size && !packet->data))
	{
		...
		if (out_samples > out_samples_max)
		{
			...
			if (e < 0)
			{
				fprintf(stderr, "[eplayer3] av_samples_alloc: %d\n", -e);
				break;
			}
