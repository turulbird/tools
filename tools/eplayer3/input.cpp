...
static void log_callback(void *ptr __attribute__ ((unused)), int lvl __attribute__ ((unused)), const char *format, va_list ap)
{
	...
	if (sizeof(m) - 1 > (unsigned int) vsnprintf(m, sizeof(m), format, ap))
	{
		...
		{
			if (lastlog_repeats)
				fprintf(stderr, "[eplayer3 input] last message repeated %u times\n", lastlog_repeats);
			lastlog_message = m;
			lastlog_repeats = 0;
			fprintf(stderr, "[eplayer3 input] %s", m);
		}
		else
		{
			lastlog_repeats++;
...
bool Input::Play()
{
	...
	while (player->isPlaying && !player->abortRequested)
	{
		...
		if (player->isPaused)
		{
			fprintf(stderr, "[eplayer3 input] paused\n");
			usleep(100000);
			continue;
		}
		...
		if (_videoTrack && (_videoTrack->stream == stream))
		{
			...
			{
				logprintf("[eplayer3 input] writing data to %s device failed\n", "video");
			}
		}
...
bool Input::Init(const char *filename, std::string headers)
{
	...
	if (!filename)
	{
		fprintf(stderr, "[eplayer3 input] filename NULL\n");
		return false;
	}

	if (!headers.empty())
	{
		fprintf(stderr, "[eplayer3 input] %s %s %d: %s\n%s\n", FILENAME, __func__, __LINE__, filename, headers.c_str());
		headers += "\r\n";
	}
	else
	{
...
bool Input::UpdateTracks()
{
	...
	for (unsigned int n = 0; n < avfc->nb_streams; n++)
	{
		...
		switch (stream->codec->codec_type)
		{
			...
			case AVMEDIA_TYPE_SUBTITLE:
			{
				...
				else
				{
					if (!stream->codec->codec)
					{
						...
						if (!stream->codec->codec)
						{
							fprintf(stderr, "[eplayer3 input] avcodec_find_decoder failed for subtitle track %d\n", n);
						}
						else
						{
			...
			default:
			{
				fprintf(stderr, "[eplayer3 input] not handled or unknown codec_type %d\n", stream->codec->codec_type);
				break;
			}
		}
	}
