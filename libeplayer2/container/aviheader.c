...
//      printf("\nindex to %p !!!!! (priv=%p)\n",priv->idx,priv);
		...
		else if ((int)chunksize < 0) aviheader_printf("chunksize=%u  (id=%.4s)\n", chunksize, (char *) &id);
		printf("%s::%d\n", __FUNCTION__, __LINE__);
	}
	printf("%s::%d\n", __FUNCTION__, __LINE__);
			...
			if (idxfix_divx)
				if (avi_stream_id(id) == idxfix_videostream)
				{
					switch (idxfix_divx)
					{
						...
						case 1:
							if (c & 0x40000000) idx->dwFlags &= ~AVIIF_KEYFRAME;
							break; // divx 3
						case 2:
							if (c == 0x1B6) idx->dwFlags &= ~AVIIF_KEYFRAME;
							break; // divx 4
					}
				}
